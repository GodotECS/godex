...
void Pipeline::build() {
	...
	for (uint32_t i = 0; i < systems_info.size(); i += 1) {
		...
		for (uint32_t c = 0; c < info.mutable_components.size(); c += 1) {
			const bool is_event = ECS::is_component_events(info.mutable_components[c]);
			if (is_event) {
				// Make sure it's unique
				if (event_generator.find(info.mutable_components[c]) == -1) {
					event_generator.push_back(info.mutable_components[c]);
				}
			}
		}
...
void Pipeline::dispatch(World *p_world) {
	...
	// Clear any generated component event storages.
	for (uint32_t c = 0; c < event_generator.size(); c += 1) {
		Storage *s = p_world->get_storage(event_generator[c]);
		if (s) {
			s->clear();
		}
	}
